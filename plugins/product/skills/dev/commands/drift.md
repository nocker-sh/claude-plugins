# /product:dev:dev drift

仕様書（README / .docs/ 配下）と実装の乖離を検出する。「実装済」と書かれているが動かない、「未実装」と書かれているが動く、テーブル数・画面数・ロール権限の食い違いを洗い出す。

## 起動条件

`/product:dev drift` 単独、または `/product:dev drift {対象範囲}` で起動する。

- `/product:dev drift` — 全 docs と全実装の突合
- `/product:dev drift features` — 機能仕様だけ
- `/product:dev drift schema` — データモデル（DATA セクション、または `models/` 配下の分割ファイル）だけ
- `/product:dev drift roles` — ロール権限だけ
